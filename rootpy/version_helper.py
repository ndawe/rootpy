...
"""
Utilities for generating the version string for rootpy (or an affiliated
package) and the version.py module, which contains version info for the
package.

Within the generated rootpy.version module, the `major`, `minor`, and `bugfix`
...
is a development version of rootpy. For the actual version string, use::

    from rootpy.version import version
...
try:
    from rootpy.utils._compiler import compiler
except ImportError:
    compiler = "unknown"
"""[1:]
...
def _get_version_py_str(packagename, version, release, debug):
    ...
    if packagename.lower() == 'rootpy':
        packagename = 'rootpy'
    else:
        packagename = 'rootpy-affiliated package ' + packagename
