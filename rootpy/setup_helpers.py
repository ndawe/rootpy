...
setup/build/packaging that are useful to rootpy as a whole.
"""
...
from .tests.helper import rootpy_test
...
class rootpyBuild(DistutilsBuild):
    """
...
class rootpyInstall(DistutilsInstall):
    """
...
rootpyBuild.__name__ = 'build'
rootpyInstall.__name__ = 'install'
...
def wrap_build_ext(basecls=DistutilsBuildExt):
    ...
    def run(self):
        from rootpy.version import release
...
for option in [
    ...
    rootpyBuild.add_build_option(*option)
    rootpyInstall.add_install_option(*option)


if HAVE_SPHINX:
    class rootpyBuildSphinx(SphinxBuildDoc):
        """ A version of the ``build_sphinx`` command that uses the
        version of rootpy that is built by the setup ``build`` command,
        rather than whatever is installed on the system - to build docs
        against the installed version, run ``make html`` in the
        ``rootpy/docs`` directory.
        ...
        description = 'Build Sphinx documentation for rootpy environment'
        ...
        def run(self):
            ...
            #Now make sure rootpy is built and determine where it was built
            ...
            subproccode = rootpyBuildSphinx._self_iden_rex.split(subproccode)
    ...
    rootpyBuildSphinx.__name__ = 'build_sphinx'
...
def get_debug_option():
    ...
    try:
        from rootpy.version import debug as current_debug
    except ImportError:
        current_debug = None
...
def update_package_files(srcdir, extensions, package_data, packagenames,
    ...
    setup.py rather than returning new ones.  See rootpy's own
    ``setup.py`` for example usage and the rootpy development docs
    for more details.
    """
...
def check_numpy():
    ...
    if not requirement_met:
        msg = "numpy version 1.4 or later must be installed to build rootpy"
    ...
    if 'CC' in os.environ:
        ...
        for broken, fixed in compiler_mapping:
            if re.match(broken, version):
                print("Compiler specified by CC environment variable ({0:s}: {1:s}) will fail to compile rootpy. Please set CC={2:s} and try again. You can do this for example by doing:\n\n    CC={2:s} python setup.py <command>\n\nwhere <command> is the command you ran.".format(c_compiler, version, fixed))
                sys.exit(1)
...
def is_in_build_mode():
    ...
    `set_build_mode`
    """
    #_rootpy_SETUP_ is added to the builtins in setup.py or rootpy/__init__.py
    return _rootpy_SETUP_
...
def set_build_mode(val=True):
    ...
    builtins._rootpy_SETUP_ = val


def setup_test_command(package_name):
    return type(package_name + '_test_command', (rootpy_test,),
...
def import_file(filename):
    ...
    # results in a number of "Parent module 'rootpy' not found while
...
# relative imports in rootpy that go above the level of the
...
from rootpy import __version__
__version__ = {equiv_version!r} + '-' + __version__
{extras}

_is_rootpy_legacy_alias = True
"""
...
def add_legacy_alias(old_package, new_package, equiv_version, extras={}):
    ...
    *pkgto*.
    ...
    For example::
       ...
       add_legacy_alias('rootpy.io.votable', 'vo')
    ...
    try:
        ...
        # may cause importing of code within the rootpy source tree,
        ...
        if os.path.exists(filename):
            with open(filename, 'U') as fd:
                if '_is_rootpy_legacy_alias' in fd.read():
                    found_legacy_module = False
